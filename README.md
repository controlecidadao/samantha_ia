...
✅ **"#IDE" feature:** Users can select and run any Python code that uses the libraries installed in the `jupyterlab` virtual environment by entering the `#IDE` comment in the code (even for code outside of Samantha), copying it and clicking the "Run Code" button;<br><br>
...
* To create a new line in a field, press SHIFT + ENTER. If only the ENTER key is pressed, the loading/processing/generation phases will begin.
