...
✅ **Edit, Copy and Run Python Code:** The system allows the user to edit the code generated by the model and run it by selecting, copying with `CTRL + C` and clicking the _Run copied code_ button. You can also copy a Python code from anywhere (e.g. from a webpage) and run it just by pressing _Run copied code_ button (as long as it uses the installed Python libraries).<br><br>

✅ **Code Blocks Editing:** Users can select and run Python code blocks generated by the model that uses the libraries installed in the `jupyterlab` virtual environment by entering the `#IDE` comment in the output code, selecting and copying with `CTRL + C`, and finally clicking the _Run copied code_ button;<br><br>
...
✅ **Stop Condition:** Stops Samantha if the automatic execution of the Python code generated by the model prints a value other than '' (empty string) in the terminal.<br><br>
...
The quality of the responses generated by a model can be evaluated using some basic criteria, such as:
