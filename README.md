...
You can select any directory that contains GGUF models. In this case, the models contained in the selected directory will be listed in the _Model selection_ dropdown menu.
...
In turn, when deactivated, it always uses the text contained in the _Assistant previous response_ field as the previous response, unless the text is preceded by `---` (triple dash).
...
* The graph window generated by the matplotlib module must be closed for another to be displayed.
