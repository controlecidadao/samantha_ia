...
✅ **Run Code Button:** The system allows the user to edit and run the code generated by the model simply by clicking the “Run Code” button. The output will be displayed in an HTML pop-up window;<br><br>
...
`n_ctx` stands for _number of context tokens_ and determines the maximum length of the context window that the model can process at once.

The context length directly impacts the memory usage and computational load. Longer context lengths require more memory and computational power.

It determines how much previous text the model can "remember" and utilize when selecting the next token from model vocabulary.

ATTENTION: `n_ctx` > `max_tokens` + number of input tokens (system prompt + assistant previous response + user prompt). 

As a rule, set `n_ctx` equals to `max_tokens` and both as higher as possible. Samantha's default value for `n_ctx` is 4000 

Before adjusting `n_ctx`, unload the model by clicking _Unload model_ button.
...
Controls maximum number of tokens to be generated in the response.
