...
✋ When checked, stops Samantha when the automatic execution of the Python code generated by the model prints (in the terminal) a value other than `''` (empty string).<br><br>
...
This button executes the code generated by the model in the last response using the Python interpreter and respective libraries installed in the `jupyterlab` virtual environment.

The `pip` and `!pip` instructions present in the code generated by the model are ignored.

Python code blocks in the last response, delimited by ` ```python ` and ` ``` `, are concatenated sequentially.

You can copy (CTRL + C) any Python script - even outside Samantha interface - that uses the libraries installed in the `jupyterlab` virtual environment and press the _Run Code_ button to execute the copied code.
