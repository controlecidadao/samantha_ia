...
Each model in the _model list_ responds to all prompts in the _user prompt list_ for the selected _number of responses_. This block is repeated for the selected _nubmer of loops_.
...
When checked, runs the code generated by the model automatically.
...
"n_ctx" stands for "number of context tokens" and determines the maximum length of the context window that the model can process at once.
...
It determines how much previous text the model can "remember" and utilize when generating new text.

Before adjusting, unload the model.
